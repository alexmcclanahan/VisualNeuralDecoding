...
areas = ['LGd', 'LP', 'CA1', 'CA3']
areas_description = 'Subcortical Indiv. Assortment'
########################################################################
# INPUT (Bin duration in ms) REQUIRED HERE: #
bin_duration_in_msec = 50
########################################################################
# Input (regions) below: #
for subregion in range(len(areas)):
########################################################################

    print(str(areas[subregion]))
    new_folder = str(areas[subregion]) + '_' + str(bin_duration_in_msec) + '_ms'
    ...
    for session_of_interest in range(len(all_sessions)):
        ...
        units = decoding_functions.fetch_units(session, areas=str(areas[subregion]))
